...
class ProposedModel(nn.Module):
    ...
    def forward(self, lan1, lan2, pad_idx, eos_idx, loop_cond):
        ...
        if loop_cond == 'start1_from_lan1_to_lan2':
            ...
            '''
                ...
                results:
                    output - (batch, lan2_seq_len, d_model)
                    output_prob - (batch, lan2_seq_len, vocab_size)
            '''
            output = self.transformer1(lan1, lan2, pad_idx, eos_idx)
            ...
            '''
                args: lan2, lan1
                    ...
                    lan1 - (batch, lan1_seq_len)
                results:
                    output - (batch, lan1_seq_len, d_model)
                    output_prob - (batch, lan1_seq_len, vocab_size)
            '''
            output = self.transformer2(lan2, lan1, pad_idx, eos_idx)
            output_prob = self.lan1_linear(output)

        elif loop_cond == 'strat2_from_lan2_to_lan1':
            # lan2 -> lan1
            '''
                args: lan2, lan1
                    lan2 - (batch, lan2_seq_len)
